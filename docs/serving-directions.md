...
currently uses 20-meter connected-component clustering, so an opposing pair
can indicate an identity issue requiring an audited migration rather than UI
...
The heading display is now provenance-correct, but it intentionally does not
repair physical-stop identity. The current distance-only builder can merge
opposite curbside members and can form clusters transitively. Stop 935 is the
reference case: member/GTFS stop 7867 has heading 119 degrees and member/GTFS
stop 7868 has heading 297 degrees. Both remain displayed because both are real
members of the current physical stop.

An audited identity migration remains separate work. It must provide split and
merge lineage, intentional handling for old URLs, deterministic future rebuilds,
and explicit treatment of observations, assignments, consensus, and stewardship
