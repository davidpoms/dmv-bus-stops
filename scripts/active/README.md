...
| `set_reviewer_role.py --db <db> --reviewer-id <id> --role reviewer\|review_lead\|owner` | assign pilot roles, including owner bootstrap | explicit database; repository production path additionally requires `--allow-production-database`; browser can never assign owner |
